...
class IBookingRepository(IBaseRepository[Booking, BookingCreateSchema, BookingUpdateSchema], ABC):
    pass


class BookingRepositroy(IBookingRepository, BaseRepository[Booking, BookingCreateSchema, BookingUpdateSchema]):
    ...
    def _map_to_entity(self, booking: BookingCreateSchema) -> Booking:
        return Booking(id=booking.id, user_id=booking.user_id, hotel_id=booking.hotel_id, check_in=booking.check_in, check_out=booking.check_out, total_price=booking.total_price, status=booking.status)
